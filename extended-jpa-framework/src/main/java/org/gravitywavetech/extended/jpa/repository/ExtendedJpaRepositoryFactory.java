...
public class ExtendedJpaRepositoryFactory extends JpaRepositoryFactory {
    ...
    protected RepositoryComposition.RepositoryFragments getRepositoryFragments(RepositoryMetadata metadata) {
        ...
        if (ExtendedBaseRepository.class.isAssignableFrom(repositoryInterface)) {
            return fragments;
        }
        ...
        // 按需装配：只注入仓储接口显式声明的片段。
        // 例：只 extends HqlQueryFragment 的仓储不会拿到 Native / Criteria 能力。
        RepositoryFragment<?> nativeF = NativeQueryFragment.class.isAssignableFrom(repositoryInterface)
                ? nativeFragment(domainType) : null;
        RepositoryFragment<?> hqlF = HqlQueryFragment.class.isAssignableFrom(repositoryInterface)
                ? hqlFragment(domainType) : null;
        RepositoryFragment<?> criteriaF = CriteriaQueryFragment.class.isAssignableFrom(repositoryInterface)
                ? criteriaFragment(domainType) : null;

        if (nativeF == null && hqlF == null && criteriaF == null) {
            return fragments;
        }

        RepositoryComposition.RepositoryFragments appended = fragments;
        if (nativeF != null) {
            appended = appended.append(nativeF);
        }
        if (hqlF != null) {
            appended = appended.append(hqlF);
        }
        if (criteriaF != null) {
            appended = appended.append(criteriaF);
        }
        return appended;
    }
