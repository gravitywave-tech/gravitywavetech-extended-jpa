...
public class NativeQueryBuilder<T> {
    ...
    public NativeQueryBuilder<T> from(String tableName, String alias) {
        sql.append("FROM ").append(tableName).append(" ").append(alias).append(" ");
        return this;
    }
    ...
    public NativeQueryBuilder<T> leftJoin(String tableName, String alias, String onCondition) {
        sql.append("LEFT JOIN ").append(tableName).append(" ").append(alias)
    ...
    public NativeQueryBuilder<T> innerJoin(String tableName, String alias, String onCondition) {
        sql.append("INNER JOIN ").append(tableName).append(" ").append(alias)
                .append(" ON ").append(onCondition).append(" ");
        return this;
    }
    ...
    public NativeQueryBuilder<T> eq(String field, Object value) {
        if (value != null) {
            where();
            sql.append(field).append(" = ?").append(params.size() + 1).append(" ");
            params.add(value);
        }
        return this;
    }
    ...
    public NativeQueryBuilder<T> eqWithAlias(String alias, String field, Object value) {
        if (value != null) {
            where();
            sql.append(alias).append(".").append(field).append(" = ?").append(params.size() + 1).append(" ");
            params.add(value);
        }
        return this;
    }
    ...
    public NativeQueryBuilder<T> inWithAlias(String alias, String field, Collection<?> values) {
        if (values != null && !values.isEmpty()) {
            where();
            sql.append(alias).append(".").append(field).append(" IN (");
            int index = 0;
            for (Object value : values) {
                if (index > 0) {
                    sql.append(", ");
    ...
    public NativeQueryBuilder<T> like(String field, String value) {
        if (StringUtils.hasText(value)) {
            where();
            sql.append(field).append(" LIKE ?").append(params.size() + 1).append(" ");
            params.add("%" + value + "%");
        }
        return this;
    }
    ...
    public NativeQueryBuilder<T> likeWithAlias(String alias, String field, String value) {
        if (StringUtils.hasText(value)) {
            where();
            sql.append(alias).append(".").append(field).append(" LIKE ?").append(params.size() + 1).append(" ");
            params.add("%" + value + "%");
        }
        return this;
    }
    ...
    public NativeQueryBuilder<T> between(String field, Object start, Object end) {
        if (start != null && end != null) {
            where();
            sql.append(field).append(" BETWEEN ?").append(params.size() + 1);
            params.add(start);
    ...
    public NativeQueryBuilder<T> orderByWithAlias(String alias, String field, String direction) {
        sql.append("ORDER BY ").append(alias).append(".").append(field).append(" ").append(direction).append(" ");
        return this;
    }
    ...
    /**
     * 执行查询并返回 Map 列表（带列名）
     * 优先使用 Hibernate ResultTransformer（性能优），失败时退回 JPA Tuple 方式
     */
    @SuppressWarnings({"unchecked", "deprecation"})
    public List<Map<String, Object>> listForMapWithAlias() {
        Query query = build();
        try {
            // 优先尝试 Hibernate API（性能更好，列名映射准确）
            org.hibernate.query.Query<?> hibernateQuery = query.unwrap(org.hibernate.query.Query.class);
            hibernateQuery.setResultTransformer(org.hibernate.transform.Transformers.ALIAS_TO_ENTITY_MAP);
            return (List<Map<String, Object>>) hibernateQuery.getResultList();
        } catch (Exception e) {
            // 兜底：使用 JPA 标准 Tuple 方式
            return listForMapWithAliasByTuple(query);
        }
    }

    /**
     * 使用 JPA 标准 Tuple 接口实现 Map 转换（可移植方案）
     */
    @SuppressWarnings("unchecked")
    private List<Map<String, Object>> listForMapWithAliasByTuple(Query query) {
        try {
            Query tupleQuery = build();
            ...
            return tuples.stream()
                    .map(tuple -> {
                        ...
                        for (TupleElement<?> element : tuple.getElements()) {
                            ...
                            map.put(alias != null && !alias.isEmpty() ? alias : "col_" + map.size(), tuple.get(element));
                        }
                        return map;
                    })
                    .collect(Collectors.toList());
